'use client'

import React, { useEffect, useMemo, useState } from 'react';
import { useRouter } from 'next/navigation';
import { getAllEntities, getAllTransactions, getAllGroupExpenses, getAllBankAccounts, resolveTransactionBankAccountId } from '../lib/db';
import type { Entity, BankAccount } from '../lib/types';
import { Box, Card, CardActionArea, CardContent, Stack, TextField, Typography } from '@mui/material';

function formatCents(cents: number) {
  const sign = cents < 0 ? '-' : '';
  const a = Math.abs(cents);
  const euros = Math.floor(a / 100);
  const rem = Math.abs(a % 100).toString().padStart(2, '0');
  return `${sign}${euros},${rem} €`;
}

export default function EntitiesPage() {
  const [entities, setEntities] = useState<Entity[]>([]);
  const [bankAccounts, setBankAccounts] = useState<BankAccount[]>([]);
  const [totals, setTotals] = useState<Record<string,{sent:number;received:number;groups:number;accounts:number}>>({});
  const [search, setSearch] = useState('');
  const router = useRouter();

  useEffect(() => {
    async function load() {
      const [es, txs, groups, accounts] = await Promise.all([getAllEntities(), getAllTransactions(), getAllGroupExpenses(), getAllBankAccounts()]);
      const accountIdsByEntity = new Map<string, BankAccount[]>();
      for (const account of accounts) {
        const list = accountIdsByEntity.get(account.entityId) ?? [];
        list.push(account);
        accountIdsByEntity.set(account.entityId, list);
      }

      const tmap: Record<string,{sent:number;received:number;groups:Set<string>;accounts:number}> = {};
      for (const e of es) tmap[e.id] = { sent:0, received:0, groups: new Set(), accounts: accountIdsByEntity.get(e.id)?.length ?? 0 };

      for (const t of txs) {
        const accountId = resolveTransactionBankAccountId(t);
        if (!accountId) continue;
        const account = accounts.find((item) => item.id === accountId);
        if (!account) continue;
        const rec = tmap[account.entityId];
        if (!rec) continue;
        if (t.amount < 0) rec.sent += t.amount; else rec.received += t.amount;
      }

      for (const g of groups) {
        // Track group participation through both expenses and refunds
        const allGroupTransactionIds = [...g.expenseTransactionIds, ...g.refundTransactionIds];
        for (const txnId of allGroupTransactionIds) {
          const pt = txs.find(x => x.id === txnId);
          const accountId = pt ? resolveTransactionBankAccountId(pt) : null;
          if (!accountId) continue;
          const account = accounts.find((item) => item.id === accountId);
          if (!account) continue;
          const rec = tmap[account.entityId];
          if (!rec) continue;
          rec.groups.add(g.id);
        }
      }

      const totals: Record<string,{sent:number;received:number;groups:number;accounts:number}> = {};
      for (const [id, rec] of Object.entries(tmap)) {
        totals[id] = { sent: rec.sent, received: rec.received, groups: rec.groups.size, accounts: rec.accounts };
      }

      setEntities(es);
      setBankAccounts(accounts);
      setTotals(totals);
    }
    load();
  }, []);

  const filteredEntities = useMemo(() => {
    const needle = search.trim().toLowerCase();
    if (!needle) return entities;
    return entities.filter((entity) => {
      const accountNames = bankAccounts
        .filter((account) => account.entityId === entity.id)
        .map((account) => account.name)
        .join(' ')
        .toLowerCase();
      return entity.name.toLowerCase().includes(needle) || accountNames.includes(needle);
    });
  }, [bankAccounts, entities, search]);

  return (
    <Box sx={{ p: 2 }}>
      <Typography variant="h5" sx={{ mb: 2 }}>Entities</Typography>

      <TextField
        fullWidth
        value={search}
        onChange={(e) => setSearch(e.target.value)}
        label="Search entities"
        placeholder="Filter by name or bank name"
        sx={{ mb: 2 }}
      />

      <Stack spacing={2}>
        {filteredEntities.map(e => {
            const t = totals[e.id] ?? { sent:0, received:0, groups:0, accounts:0 };
          return (
            <Card key={e.id} variant="outlined">
              <CardActionArea onClick={() => router.push(`/entities/${e.id}`)}>
                <CardContent sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 2 }}>
                  <Box sx={{ minWidth: 0 }}>
                    <Typography sx={{ fontWeight: 600 }} noWrap>{e.name}</Typography>
                  </Box>
                  <Box sx={{ textAlign: 'right', flexShrink: 0 }}>
                    <Typography variant="body2">Sent: <strong>{formatCents(t.sent)}</strong></Typography>
                    <Typography variant="body2">Received: <strong>{formatCents(t.received)}</strong></Typography>
                  </Box>
                </CardContent>
              </CardActionArea>
            </Card>
          );
        })}
      </Stack>
    </Box>
  );
}
